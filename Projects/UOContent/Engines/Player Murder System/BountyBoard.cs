...
{
    // Offset added to player serials to produce synthetic item serials for BB packets.
    // High enough that no real item will have this serial, reversible by subtraction.
    private const uint SyntheticSerialBase = 0x7F000000;
        ...
        switch (packetID)
        {
            ...
            case 4: // Request header
            {
                ...
                if (World.FindMobile(playerSerial) is PlayerMobile player)
                {
                    ...
                    if (bounty > 0)
                    {
                        SendBountyMessage(from.NetState, this, (Serial)syntheticSerial, player, bounty, packetID == 3);
                    }
                }
    ...
    private static void SendBountyMessage(
        NetState ns, BaseBulletinBoard board, Serial messageSerial, PlayerMobile player, int bounty, bool content)
    {
        if (ns.CannotSendPackets())
        {
            return;
